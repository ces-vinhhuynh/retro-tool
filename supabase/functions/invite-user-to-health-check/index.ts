...
serve(async (req) => {
  ...
  try {
    const payload = (await req.json()) as HealthCheckInvitePayload;

    if (!payload.userIds || !payload.healthCheckId) {
      return new Response(
        JSON.stringify({
          success: false,
    ...
    const result = await handleHealthCheckInvitation(payload);
